...
GOALS OF THIS SCRIPT:
    ...
    --Define the top and bottom of each sankey curve for Tableau to plot polygons

    - Add in spacer data for spaces in between real source/function curves
    - Add in dummy category data to add as spacers for inner R/O pillars
...
project_dir = main_dir + "/MTS_JMB_Only/tableau visualizations/_Sankey_Cover_Figure_Revamp" #sankey folder
...
path = data_dir + "/fig_cover_0117_made_171005.csv" 
...
""" Setting dummy amount-----------------------------------------------------"""
dummy_amt = sum_outlays/4 # hard coding the dummy amount for now --> could change this later
"""-----------------------------------------------------Setting dummy amount"""
...
# SET ALL SPACE VALUES TO AN EQUAL VALUE --------------------------------------
    ...
    # This part has been modified multiple times, and may need to be adjusted for aesthetic preferences
...
if len(df_r['Stage2_Receipt_Type'].str.contains("Deficit").unique().tolist()) > 1: # if there is a deficit
    ...
    deficit = int(df_receipts['Stage1_Source_Amount'][index_def])
else:
    index_surp = df_outlays[(df_outlays.loc[:, 'Stage3_Outlay_Type']=="Surplus")].index.tolist()[0]
    surplus = int(df_outlays['Stage3_Outlay_Amount'][index_surp])
    

# Dummy Amount was hard-coded above fraction of all outlays
dummy_amt

# Setting Recipts Spacers -----------------------------------------------------
...
# SETTING RECEIPT SIDE SPACER VALUES - ALL EQUAL SIZE
if len(df_r['Stage2_Receipt_Type'].str.contains("Deficit").unique().tolist()) > 1: # if there's a deficit
    total_rec_spacer_amount = (2*dummy_amt + deficit) 
else:
     total_rec_spacer_amount = (2*dummy_amt) #if surplus, it should be 2*dummy

size_rec_spacer = np.round(int(total_rec_spacer_amount/num_receipt_spacers)) 
...
# SETTING OUTLAY SIDE SPACER VALUES - ALL EQUAL SIZE

# How many spacer groups do we need for outlays?
if len(df_r['Stage2_Receipt_Type'].str.contains("Deficit").unique().tolist()) > 1: # if there's a deficit
    num_outlay_spacers = len(df_outlays) - 1 #only need spacers between real functions
else:
     num_outlay_spacers = len(df_outlays) - 2 # if a surplus, subtract one more group


# SETTING OUTLAY SIDE SPACER VALUES - ALL EQUAL SIZE
if len(df_r['Stage2_Receipt_Type'].str.contains("Deficit").unique().tolist()) > 1: 
    total_out_spacer_amount = (2*dummy_amt) # if there's a deficit, outlay space should be 2*dummy
else:                   
     total_out_spacer_amount = (2*dummy_amt + surplus) # if there's a surplus, outlay space should be 2*dummy + surplus

size_out_spacer = np.round(int(total_out_spacer_amount/num_outlay_spacers)) 

# Force the value of each spacer or leave Real data's amount as is
for i in range(len(df_out)):
    if df_out['Stage4_Function'][i].startswith("Spacer"):
        df_out['Stage3_Outlay_Amount'][i] = size_out_spacer
...
# For print out message at the end so you can copy-paste into Tableau 
    # Get total outlays, total receipts, surplus, and deficit for floating objects on dashbaord.
df_fig_cov = pd.read_csv(path)
df_fig_cov.head()


# Total outlays
index_total_outlays = df_fig_cov[(df_fig_cov.loc[:, 'source_func']=="Total") &
                                 (df_fig_cov.loc[:, 'outlay']==True)].index.tolist()[0]
total_outlays = int(df_fig_cov['amt'][index_total_outlays])

# Total receipts
index_total_receipts = df_fig_cov[(df_fig_cov.loc[:, 'source_func']=="Total") &
            (df_fig_cov.loc[:, 'rec']==True)].index.tolist()[0]
total_receipts = int(df_fig_cov['amt'][index_total_receipts])
...
print("")
print("########################################################################")
if len(df_r['Stage2_Receipt_Type'].str.contains("Deficit").unique().tolist()) > 1: # if there was a surplus
    print("This month, " + curr_mo +"-"+ curr_fy + ", there was a deficit. You should use the Tableau workbook for deficits.")
    print("Deficit was: " + "$"+"{:,}".format(int(deficit)))
else: 
    print("This month, " + curr_mo +"-"+ curr_fy + ", there was a surplus. You should use the Tableau workbook for surpluses.")
    print("Surplus was: " + "$"+"{:,}".format(int(surplus)))
        
print("Total receipts were: " + "$"+"{:,}".format(int(total_receipts)))
print("Total outlays were: " + "$"+"{:,}".format(int(total_outlays)))
